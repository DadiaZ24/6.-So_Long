...
/*   Updated: 2024/07/08 17:01:17 by ddias-fe         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	map_parser(t_event *event)
{
	...
	if (!parse_stuff(event))
		return (ft_printf(ERROR_MAP_CHECK, BLUE, ERROR_TOP, RED, DEFAULT_COLOR,
				BLUE, DEFAULT_COLOR, BLUE, DEFAULT_COLOR, BLUE, DEFAULT_COLOR,
	...
	if (!check_square(event))
		return (ft_printf(ERROR_MAP_SQUARE, BLUE, ERROR_TOP, RED, DEFAULT_COLOR,
				BLUE, ERROR_BOT, DEFAULT_COLOR), 0);
		...
		while (++event->parser.x < event->map_columns)
		{
			...
			if (event->map[event->parser.y][event->parser.x] == 'C')
				if (!parse_check_access(event, 'C'))
					return (0);
			if (event->map[event->parser.y][event->parser.x] == 'X')
				if (!parse_check_access(event, 'X'))
					return (0);
		}
	}
	if (event->parser.players != 1 || event->parser.colects < 1
		|| event->parser.exit != 1 || event->parser.enemies < 1)
		return (0);
	return (1);
}
...
int	parse_check_access(t_event *event, char etype)
{
	...
	{
		event->parser.enemies += 1;
		event->parser.path_enemies += 1;
	}
	else
		return (0);
	return (1);
